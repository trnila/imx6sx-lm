...
while True:
    try:
        ...
        if frame.type == PLINMessageType.FRAME:
            flags = [flag.name for flag in PLINFrameErrorFlag if flag in PLINFrameErrorFlag(frame.flags)]
            print(f"{frame.id:02x}: {bytearray(frame.data[:frame.len]).hex(' ')} {','.join(flags)}")
